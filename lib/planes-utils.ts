...
const planesDeEstudios: PlanDeEstudios[] = []

export function enrichAsignaturasWithPlanInfo(asignaturas: any[]): AsignaturaConPlan[] {
  return asignaturas.map(asignatura => {
    // Buscar información del plan para esta asignatura
    const planInfo = planesDeEstudios.find(plan => 
      plan.nombre.toLowerCase() === asignatura.materia.toLowerCase() ||
      plan.nombreCorto.toLowerCase() === asignatura.materia.toLowerCase() ||
      plan.siglas.toLowerCase() === asignatura.materia.toLowerCase()
    )

    // Determinar orientaciones basadas en el tipo de asignatura y otros factores
    let orientaciones: string[] = []
    
    if (asignatura.tipoAsignatura === "Seminario PST") {
      orientaciones = ["Profesorado"]
    } else if (asignatura.materia.toLowerCase().includes("arqueolog")) {
      orientaciones = ["Licenciatura - Orientación Arqueológica"]
    } else if (asignatura.materia.toLowerCase().includes("sociocultural")) {
      orientaciones = ["Licenciatura - Orientación Sociocultural"]
    } else {
    ...
    return {
      ...
      planInfo: {
        orientaciones,
        correlatividad: planInfo?.correlatividad,
...
export function getAsignaturasPorCiclo(asignaturas: AsignaturaConPlan[], ciclo: string) {
  return asignaturas.filter(asignatura => 
    asignatura.planInfo?.ciclo === ciclo
  )
}

export function filtrarAsignaturasPorOrientacion(asignaturas: AsignaturaConPlan[], orientacion: string) {
  return asignaturas.filter(asignatura =>
    asignatura.planInfo?.orientaciones.includes(orientacion)
  )
}

export function getOrientacionesDisponibles(asignaturas: AsignaturaConPlan[]): string[] {
  const orientaciones = new Set<string>()
  
  asignaturas.forEach(asignatura => {
    asignatura.planInfo?.orientaciones.forEach(orient => {
      orientaciones.add(orient)
    })
  })
  
  return Array.from(orientaciones).sort()
}

export function actualizarPlanesDeEstudios(planes: PlanDeEstudios[]) {
  planesDeEstudios.splice(0, planesDeEstudios.length, ...planes)
}
